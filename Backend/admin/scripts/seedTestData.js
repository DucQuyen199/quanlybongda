  ...
  try {
    ...
    for (const player of testData.players) {
      try {
        ...
        const checkResult = await pool.request()
          .input('id', sql.VarChar, player.id)
          .query('SELECT MaCauThu FROM CauThu WHERE MaCauThu = @id');
        
        if (checkResult.recordset.length === 0) {
          await pool.request()
            .input('id', sql.VarChar, player.id)
            .input('name', sql.NVarChar, player.name)
            .input('birthDate', sql.Date, player.birthDate)
            .input('position', sql.NVarChar, player.position)
            .input('number', sql.Int, player.number)
            .input('teamId', sql.VarChar, player.teamId)
            .query(`
              INSERT INTO CauThu (MaCauThu, HoTen, NgaySinh, ViTri, SoAo, MaDoi)
              VALUES (@id, @name, @birthDate, @position, @number, @teamId)
            `);
